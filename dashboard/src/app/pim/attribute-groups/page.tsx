...
export default function AttributeGroupsPage() {
  ...
  const { data: groups, mutate } = useSWR("/api/v1/pim/attribute-groups", fetchAPI);

  const handleCreate = async () => {
    try {
      await apiClient.post("api/v1/pim/attribute-groups", {
      ...
      <main className="flex-1 flex flex-col h-full relative z-10 overflow-hidden pr-4">
        ...
        <div className="flex items-center justify-between px-8 py-6 flex-shrink-0">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Group attributes together to apply them in bulk to templates.</p>
          </div>
          ...
          <div className="bg-white dark:bg-[#121212] rounded-3xl border border-black/5 dark:border-white/5 shadow-sm overflow-hidden min-h-[400px]">
            {(!groups || (groups as any[]).length === 0) ? (
              <div className="h-[400px] flex flex-col items-center justify-center text-center">
                ...
                <p className="text-sm text-muted-foreground max-w-sm mt-2 mb-6">Attribute Groups allow you to bundle attributes (e.g. 'Dimensions': Length, Width, Height) and assign them to templates in one click.</p>
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="border-b border-black/5 dark:border-white/5">
                    <th className="py-4 px-6 text-xs font-semibold text-muted-foreground uppercase tracking-wider">Group Name</th>
                ...
                <tbody className="divide-y divide-black/5 dark:divide-white/5">
                  {(groups as any[]).map((group: any) => (
                    <tr key={group.id} className="hover:bg-black/5 dark:hover:bg-white/5 transition-colors group">
                      ...
                      <td className="py-4 px-6">
                        <span className="text-sm text-muted-foreground">{new Date(group.created_at).toLocaleDateString()}</span>
                      </td>
      ...
      {isCreating && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md transition-all">
          <div className="w-[440px] bg-white/90 dark:bg-[#121212]/95 backdrop-blur-xl rounded-3xl p-8 shadow-[0_24px_48px_rgb(0,0,0,0.3)] border border-white/20 dark:border-white/10 flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 zoom-in-95 duration-300">
            <div>
              <h2 className="text-2xl font-bold tracking-tight bg-gradient-to-br from-black to-black/70 dark:from-white dark:to-white/70 bg-clip-text text-transparent">New Attribute Group</h2>
              <p className="text-sm text-muted-foreground mt-1.5">Group related attributes together.</p>
            </div>

            <div className="flex flex-col gap-5">
              <div className="flex flex-col gap-2">
                <label className="text-xs font-semibold text-foreground/80 tracking-wide">Group Name <span className="text-red-500">*</span></label>
                <input 
                  type="text" 
                  value={newName}
