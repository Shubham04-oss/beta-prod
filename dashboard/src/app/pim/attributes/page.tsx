...
export default function AttributesPage() {
  const { attributes, isLoading, createAttribute } = useAttributes();
      ...
      <main className="flex-1 overflow-y-auto custom-scrollbar pr-4 flex flex-col gap-6">
        ...
        <div className="flex items-center justify-between mt-2">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Manage global product attributes and options.</p>
          </div>
        ...
        <div className="bg-white/20 dark:bg-white/5 rounded-[2rem] p-6 border border-white/30 dark:border-white/10 flex flex-col flex-1">
          ...
          <div className="w-full overflow-x-auto">
            {isLoading ? (
              <p className="text-sm text-muted-foreground p-4">Loading attributes...</p>
            ) : filteredAttributes?.length === 0 ? (
              <p className="text-sm text-muted-foreground p-4">No attributes found.</p>
            ) : (
              <table className="w-full text-sm text-left">
                <thead>
                  <tr className="text-[11px] text-muted-foreground uppercase tracking-wider border-b border-black/5 dark:border-white/5">
                    <th className="pb-3 font-semibold px-2">Attribute Name</th>
                ...
                <tbody className="divide-y divide-black/5 dark:divide-white/5">
                  {filteredAttributes?.map((attr) => (
                    <tr key={attr.id} className="group hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition-colors">
                      <td className="py-3 px-2">
                        <div className="flex items-center gap-3">
                          ...
                          <span className="font-medium text-sm text-foreground">{attr.name}</span>
                        </div>
                      </td>
                      <td className="py-3 px-2">
                        <span className="text-xs px-2 py-1 bg-black/5 dark:bg-white/5 rounded-md font-medium text-muted-foreground">{attr.slug}</span>
                      </td>
                      <td className="py-3 px-2">
                        <span className="text-xs font-semibold px-2 py-1 bg-black/5 dark:bg-white/5 rounded-md text-foreground">{attr.type}</span>
                      </td>
                      <td className="py-3 px-2 text-right">
                        <Button size="sm" variant="ghost" className="h-7 text-[10px] rounded-md">Edit Values</Button>
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      {isCreating && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-md transition-all">
          <div className="w-[440px] bg-white/90 dark:bg-[#121212]/95 backdrop-blur-xl rounded-3xl p-8 shadow-[0_24px_48px_rgb(0,0,0,0.3)] border border-white/20 dark:border-white/10 flex flex-col gap-8 animate-in fade-in slide-in-from-bottom-4 zoom-in-95 duration-300">
            <div>
              <h2 className="text-2xl font-bold tracking-tight bg-gradient-to-br from-black to-black/70 dark:from-white dark:to-white/70 bg-clip-text text-transparent">New Attribute</h2>
              <p className="text-sm text-muted-foreground mt-1.5">Add a new global product attribute.</p>
            </div>

            <div className="flex flex-col gap-5">
              <div className="flex flex-col gap-2">
                <label className="text-xs font-semibold text-foreground/80 tracking-wide">Attribute Name <span className="text-red-500">*</span></label>
                <input 
                  type="text" 
                  value={newName}
                  onChange={(e) => {
              ...
              <div className="flex flex-col gap-2">
                <label className="text-xs font-semibold text-foreground/80 tracking-wide">Type</label>
                <select
                  value={newType}
